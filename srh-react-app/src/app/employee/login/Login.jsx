...
export default function Login() {
  ...
  const resetForm = () => {
    ...
    setTouched({ email: false, password: false, role: false });
    setErrorMsg('');
  };
  ...
  const handleSubmit = async (e) => {
    ...
    const loginUrlMap = {
      ...
      department: '/api/department/login'
    };

    const url = loginUrlMap[loginData.role];
  ...
  return (
    <>
      <div className={`login-container ${modalVisible || infoVisible ? 'login-blur-background' : ''}`}>
        <div className="login-left-pane">
          ...
          <div style={{ display: 'flex', gap: '1rem', marginTop: '1rem' }}>
            <button className="login-btn-login" onClick={openModal}>
              Login
            </button>
            <button className="login-btn-info" onClick={openInfo}>
              Info
            </button>
          </div>
        </div>
      ...
      {/* Login Modal */}
      {modalVisible && (
        <div className="login-modal-overlay show" id="loginModal" role="dialog" aria-modal="true" tabIndex="-1">
          <div className="login-card">
            <button onClick={closeModal} className="login-close-btn" aria-label="Close Login Form">
              &times;
            </button>
            <h2>🔐 Login</h2>
            ...
            <form onSubmit={handleSubmit} noValidate>
              ...
              <input
                ...
                required
              />
              {touched.password && loginData.password === '' && (
              ...
              {touched.role && loginData.role === '' && (
                <div className="login-error-text">Please select a role.</div>
              )}

              <div className="login-actions">
                <button type="submit" disabled={!loginData.email || !loginData.password || !loginData.role}>
                  Submit
                </button>
            ...
            {errorMsg && (
              <p className="login-error-text login-error-msg">{errorMsg}</p>
            )}
          </div>
        </div>
      )}
      ...
      {/* Info Modal */}
      {infoVisible && (
        <div className="login-modal-overlay show" id="infoModal" role="dialog" aria-modal="true" tabIndex="-1" style={{ overflowY: 'auto' }}>
          <div className="login-card" style={{ maxWidth: '700px' }}>
            <button onClick={closeInfo} className="login-close-btn" aria-label="Close Info Modal">
              &times;
            </button>
            <h2>🛠️ Service Request Handling System</h2>
